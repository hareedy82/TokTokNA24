...
// Prepare the registration data with additional properties
let regData = {
createdAt: userDetails.createdAt ? userDetails.createdAt : Date.now(), // Use current timestamp if not provided
...
approved: true,
verified: true, // Additional property
walletBalance: 0, // Additional property
pushToken: 'init'
};
...
// Send success response with user UID
response.send({ uid: userRecord.uid });
} else {
// Send error response if user was not created
response.send({ error: "User record was not created" });
}
} catch (error) {
